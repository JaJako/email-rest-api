...
entities are mutable per se, the DTOs enable to pass immutable objects between service and clients. Further more
specialised versions for different use cases/requests can be used to allow best-possible validation by using the DTO
definitions,
e.g. [InsertNewDraftEmailDto.java](src/main/java/de/jjakobus/emailrestservice/model/dtos/InsertDraftEmailDto.java).

Details on Controller (endpoints)...
